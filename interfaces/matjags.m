function [samples, stats, structArray] = matjags_orig(dataStruct, jagsFilenm, initStructs , varargin)
% MATJAGS, a Matlab interface for JAGS
% Version 1.2. Tested on JAGS 3.2.0, Windows 64-bit version
%
...
% This could look something like "C:\Program Files\JAGS\JAGS-3.2.0\x64\bin"
%
% For MAC users:
...
if ispc
    defaultworkingDir = '.' ; % change this to fit your particular needs
elseif ismac
...
elseif isunix
    ...
    libpath = '/usr/lib/x86_64-linux-gnu/:/lib/x86_64-linux-gnu/:/lib64/' ;

    defaultworkingDir = 'jagstmp' ; % change this to fit your particular needs
end

% Get the parameters
[ nChains , workingDir , nBurnin , nSamples , ...
    monitorParams , thin , dodic , doParallel , ...
    savejagsoutput , verbosity , cleanup , ...
    showwarnings , dotranspose , modules , doboot ] =  ...
    process_options( ...
    varargin, ...
    'nChains'        ,                1  , ...
...
if ~exist( workingDir , 'dir' )
    [ success , message ] = mkdir( workingDir ) ;
    if ~success
        error( message ) ;
    end
end
...
%     cmd = sprintf( 'cat %s | xargs' , tfn ) ;
    ...
    for whchain = 1:nChains
        ...
        elseif isunix
           jagsPrefix = 'jags' ;
           cmd = sprintf( 'export LD_LIBRARY_PATH=%s; %s %s' , libpath , jagsPrefix , jagsScript ) ;
        end
        if verbosity > 0
